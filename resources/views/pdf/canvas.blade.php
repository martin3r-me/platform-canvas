...
<head>
    <meta charset="utf-8">
    <title>{{ $canvas->name }} - {{ $canvas->canvasType?->name ?? 'Canvas' }}</title>
    <style>
        @page {
            margin: 8mm 10mm;
            size: A4 landscape;
...
<body class="scale-{{ $fontScale }}">
    ...
    @if($hasAreas)
        ...
        @php
            ...
            $reverseMap = array_flip($areaMap); // area_code => block_key
            ...
            // Calculate rowspan and colspan for each unique area
            $rendered = []; // track which areas have been rendered
    ...
    <div class="footer">
        ...
        &middot; {{ $canvas->canvasType?->name ?? 'Canvas' }}
    </div>
</body>
</html>
